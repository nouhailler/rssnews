...
- En production : connexion via `DATABASE_URL` (fournie par Neon)
- Wrapper `_Conn` expose `conn.execute()` comme sqlite3 (pas de `conn.execute()` natif en psycopg2)
- `RealDictCursor` → les lignes sont des `dict` (pas de `row_factory` manuel)
- Paramètres `%s` (PostgreSQL) au lieu de `?` (SQLite)
...
- DB hébergée sur **Neon** (PostgreSQL serverless) — pas de disque local nécessaire
- Le service "spin-down" après inactivité → première requête ~30 s
- Variables d'env **obligatoires** en production :
  - `SECRET_KEY` — clé JWT
  - `DATABASE_URL` — URL PostgreSQL fournie par Neon (format `postgresql://...`)
